...
public partial class Guard
{
	...
	private partial class ThrowHelper
	{
		...
		/// </summary>
		[DoesNotReturn]
		public static void ThrowArgumentExceptionForCanRead(Stream stream, string name) => throw new ArgumentException($"Stream {AssertString(name)} ({stream.GetType().ToTypeString()}) doesn't support reading.", name);

		/// <summary>
		/// Throws an <see cref="ArgumentException"/> when <see cref="CanWrite"/> fails.
		/// </summary>
		[DoesNotReturn]
		public static void ThrowArgumentExceptionForCanWrite(Stream stream, string name) => throw new ArgumentException($"Stream {AssertString(name)} ({stream.GetType().ToTypeString()}) doesn't support writing.", name);

		/// <summary>
		/// Throws an <see cref="ArgumentException"/> when <see cref="CanSeek"/> fails.
		/// </summary>
		[DoesNotReturn]
		public static void ThrowArgumentExceptionForCanSeek(Stream stream, string name) => throw new ArgumentException($"Stream {AssertString(name)} ({stream.GetType().ToTypeString()}) doesn't support seeking.", name);
