...
public partial class Guard
{
	...
	private partial class ThrowHelper
	{
		...
		public static void ThrowArgumentExceptionForIsNotNullOrWhiteSpace(string? text, string name)
		{
			...
			static Exception GetException(string? text, string name)
				=> text is null
				? new ArgumentNullException(name, $"Parameter {AssertString(name)} (string) must not be null or whitespace, was null.")
				: new ArgumentException($"Parameter {AssertString(name)} (string) must not be null or whitespace, was whitespace.", name);
